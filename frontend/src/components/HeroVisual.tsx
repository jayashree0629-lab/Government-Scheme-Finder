...
export function HeroVisual() {
  return (
    <div className="relative mx-auto w-full max-w-[280px] py-2" aria-hidden="true">
      <div
        className="absolute left-[19px] top-5 bottom-5 w-px bg-gradient-to-b from-transparent via-accent-400/60 to-transparent"
        style={{
          backgroundImage:
      ...
      <div className="flex flex-col gap-9">
        {NODES.map((node, i) => (
          <div key={node.label} className="relative flex items-center gap-3.5">
            <span
              className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-accent-400/50 bg-navy-800 text-accent-300"
            ...
            >
              ...
              />
              <node.Icon className="h-4 w-4 relative" />
            </span>
            <span className="text-sm font-medium text-white/90">{node.label}</span>
          </div>
        ))}
      </div>
    </div>
